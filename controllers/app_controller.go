...
func (r *AppReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, app); err != nil {
		if apierrors.IsNotFound(err) {
			logger.Info("App no longer exists")
		}
	...
	processes := &cfappsv1alpha1.ProcessList{}
	//err := r.List(ctx, processes, client.InNamespace(req.Namespace), client.MatchingFields{"spec.owner": string(app.ObjectMeta.UID)})
	err := r.List(ctx, processes, client.InNamespace(req.Namespace), client.MatchingLabels{"cf4k8s.cloudfoundry.org/appGuid": app.Name})
	if err != nil {
		logger.Info(fmt.Sprintf("Error fetching processes: %s", err))
		return ctrl.Result{}, err
	}

	if app.Spec.State == "STARTED" {
		for _, process := range processes.Items {
			...
			logger.Info(fmt.Sprintf("Finished creating/updating the LRP: %s", result))
		}
	}
